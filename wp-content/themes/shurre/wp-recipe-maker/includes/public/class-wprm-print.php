...
class WPRM_Print {
	...
	public static function print_page() {
		...
		if ( $recipe_id && WPRM_POST_TYPE === get_post_type( $recipe_id ) ) {
			...
			$print_html = '<html dir="' . $dir . '"><head><meta http-equiv="Content-Type" content="text/html; charset=' . $charset . '" />';

			// Set canonical or noindex.
			if ( $recipe->parent_url() ) {
				$print_html .= '<link rel="canonical" href="' . esc_attr( $recipe->parent_url() ) . '">';
			} else {
				$print_html .= '<meta name="robots" content="noindex">';
			}

			if ( WPRM_Settings::get( 'metadata_pinterest_disable_print_page' ) ) {
				$print_html .= '<meta name="pinterest" content="nopin" />';
			}

			$print_html .= $site_icon;
			$print_html .= $styles . $scripts;
			$print_html .= '</head><body class="wprm-print' . $rtl_class . '" data-recipe="' . esc_attr( $recipe_id ) . '">';
