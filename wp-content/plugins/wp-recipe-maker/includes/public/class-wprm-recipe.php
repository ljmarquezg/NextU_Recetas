...
class WPRM_Recipe {
	...
	public function video_metadata() {
		...
		if ( $this->video_id() ) {
			...
			$image_sizes = array(
				...
				$this->video_thumb_url( 'wprm-metadata-1_1' ),
				$this->video_thumb_url( 'wprm-metadata-4_3' ),
				$this->video_thumb_url( 'wprm-metadata-16_9' ),
			);
			...
			// Check for Mediavine video.
			preg_match( '/video.mediavine.com\/videos\/(.*?)\.js/im', $embed_code, $match );
			...
			/* 
			Structured Data Testing Tool doesn't pick up on Youtube metadata.
			If they do, following code can be used.

			preg_match("/youtube.com\/embed\/(.*?)\"/im", $embed_code, $match );
			if ( $match && isset( $match[1] ) ) {
				$metadata = array(
					'@id' => 'https://www.youtube.com/watch?v=' . $match[1],
				);
			}
			*/
		}
	...
	/**
	 * Get the formatted recipe custom time.
	 *
	 * @since    2.2.0
	 * @param    boolean $shorthand Wether to use shorthand for the unit text.
	 */
